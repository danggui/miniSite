define(['ajax','EnvStore'], function (ajax,EnvStore) {
  ...
  var Model = _.inherit({
    ...
    buildurl: function () {
      ...
      //      return this.protocol + '://' + baseurl.domain + '/' + baseurl.path + (typeof this.url === 'function' ? this.url() : this.url);
        if(EnvStore.getInstance().getAttr('envUrl')!=undefined)
            this.baseurl = {
                domain: EnvStore.getInstance().getAttr('envUrl').replace(/http:\/\/|https:\/\//,''),
                path: ''
            };
        this.protocol = /https|http/.exec(EnvStore.getInstance().getAttr('envUrl'))[0];
    ...
    */
    execute: function (onComplete, onError, scope, onAbort, params) {
      ...
      var __onComplete = $.proxy(function (data,textStatus,resObj) {
        ...
        if (typeof onComplete === 'function') {
          onComplete.call(scope || this, datamodel, data,textStatus,resObj);
        }
