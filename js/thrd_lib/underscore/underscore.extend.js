  ...
  */
  _.dateUtil = {
    ...
    */
    ...
    // @description 获取一年每月份的天数集合
    // @param year {num} 可能是年份或者为一个date时间
    // @param year {num} 月份
    // @return {num} 数组
    getDaysOfYear: function (year, month) {
      //自动减一以便操作
      month--;
      ...
      return [31, _.dateUtil.isLeapYear(year) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    },
...
(function(){
  ...
  HObject.set = function(e, t, value) {
    if (!t) return null;
    var r = t.split(".");
    e = e || {};
    for (var i = 0,
             s = r.length,
             o = Math.max(s - 1, 0); i < s; i++) i < o ? e = e[r[i]] = e[r[i]] || {}: e[r[i]] = value;
    return e
  };
  HObject.get = function(e, t) {
    if (!e || !t) return null;
    var n = t.split(".");
    e = e || {};
    for (var r = 0,
...
//        n = e.shift() || {};
//    if (!n) return !1;
//    for (var r = 0, i = e.length; r < i; r++) if (typeof e[r] == "object") for (var s in e[r]) n[s] = e[r][s];
//    return n
//  }
})();
//封装setTimeout
...
(function(){
  return _.ElPosition = {
      getElementPos : function (a) {
        var b = 0,
            c = 0;
        do b += a.offsetTop, c += a.offsetLeft;
      ...
      getMousePos : function (a) {
        var b = Math.max(document.body.scrollTop, document.documentElement.scrollTop),
            c = Math.max(document.body.scrollLeft, document.documentElement.scrollLeft);
        return {
          top: b + a.clientY,
          left: c + a.clientX
        }
      ...
      getMousePosOfElement : function (b, c) {
        var d = this.getMousePos(b),
            e = this.getElementPos(c),
            f = c.clientWidth,
            g = c.clientHeight,
            h = d.left - e.left,
            i = d.top - e.top;
        return h = 0 > h ? 0 : h > f ? f : h, i = 0 > i ? 0 : i > g ? g : i, {
          x: h,
          y: i
        }
      }
...
//script async load
(function(){
  _.execScript= function(html){
    html && /\S/.test(html) && (window.execScript ||
      function(html){
        window.eval.call(window,html)
      })(html)
  }
}());
